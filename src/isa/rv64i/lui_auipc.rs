//! RV64I LUI and AUIPC Operations
//!
//! This module implements the upper immediate instructions for RV64I:
...
/// storing the result in a register. Used for PC-relative addressing.
///
/// # Arguments
/// * `instr` - Decoded instruction
...
mod tests {
    ...
    fn create_test_instr(opcode: Opcode, rd: u8, imm: u32) -> DecodedInstruction {
        DecodedInstruction {
            raw: 0,
    ...
    fn test_lui_basic() {
        ...
        let instr = create_test_instr(Opcode::Lui, 1, 0x12345000);
    ...
    fn test_lui_rd_zero() {
        let mut state = CoreState::default();
        let instr = create_test_instr(Opcode::Lui, 0, 0x12345000);
        let mut mem = SimpleMemory::new(0x1000);
    ...
    fn test_lui_negative() {
        let mut state = CoreState::default();
        // 0xFFF00000 as i32 is negative (-0x100000)
        let instr = create_test_instr(Opcode::Lui, 1, 0xFFF00000);
        let mut mem = SimpleMemory::new(0x1000);

        exec_lui(&instr, &mut state, &mut mem).unwrap();

        // Sign-extended to 64 bits
        assert_eq!(state.regs[1], 0xFFFFFFFFFFF00000);
    }
    ...
    fn test_auipc_basic() {
        ...
        let instr = create_test_instr(Opcode::Auipc, 1, 0x00001000);
        let mut mem = SimpleMemory::new(0x1000);
    ...
    fn test_auipc_rd_zero() {
        ...
        let instr = create_test_instr(Opcode::Auipc, 0, 0x12345000);
        let mut mem = SimpleMemory::new(0x1000);
    ...
    fn test_auipc_large_offset() {
        ...
        let instr = create_test_instr(Opcode::Auipc, 1, 0x12345000);
        let mut mem = SimpleMemory::new(0x1000);
    ...
    #[test]
    fn test_auipc_negative_offset() {
        let mut state = CoreState {
            pc: 0x100000,
            ..Default::default()
        };
        // 0xFFF00000 sign-extended is -0x100000
        let instr = create_test_instr(Opcode::Auipc, 1, 0xFFF00000);
        let mut mem = SimpleMemory::new(0x1000);
