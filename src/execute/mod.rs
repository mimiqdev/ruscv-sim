...
impl Executor {
    ...
    fn exec_lui(&self, instr: &DecodedInstruction, state: &mut CoreState) -> Result<(), ExecuteError> {
    ...
    fn exec_auipc(&self, instr: &DecodedInstruction, state: &mut CoreState) -> Result<(), ExecuteError> {
        if let (Some(rd), Some(imm)) = (instr.rd, instr.imm) {
            if rd != 0 {
    ...
    /// JAL (Jump and Link)
    fn exec_jal(&self, instr: &DecodedInstruction, state: &mut CoreState) -> Result<(), ExecuteError> {
        if let (Some(rd), Some(imm)) = (instr.rd, instr.imm) {
    ...
    fn exec_jalr(&self, instr: &DecodedInstruction, state: &mut CoreState) -> Result<(), ExecuteError> {
    ...
    /// 分支指令
    fn exec_branch(&self, instr: &DecodedInstruction, state: &mut CoreState) -> Result<(), ExecuteError> {
    ...
    /// 加载指令
    fn exec_load(&self, instr: &DecodedInstruction, state: &mut CoreState, mem: &dyn MemoryInterface) -> Result<(), ExecuteError> {
        let (Some(rd), Some(rs1), Some(imm), Some(funct3)) = (instr.rd, instr.rs1, instr.imm, instr.funct3) else {
            return Err(ExecuteError::InvalidOperation);
        };
    ...
    /// R-type 操作指令
    fn exec_op(&self, instr: &DecodedInstruction, state: &mut CoreState) -> Result<(), ExecuteError> {
        let (Some(rd), Some(rs1), Some(rs2), Some(funct3), Some(funct7)) = 
            (instr.rd, instr.rs1, instr.rs2, instr.funct3, instr.funct7) else {
            return Err(ExecuteError::InvalidOperation);
        };
...
mod tests {
    ...
    fn test_lui_execution() {
        ...
        let mem = SimpleMemory::new(0x1000);
