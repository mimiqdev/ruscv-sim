...
- `cargo-llvm-cov`
- Native C/C++ build tools, CMake, Ninja, Python, and GDB
- GNU `riscv64-unknown-elf` C, C++, assembler, linker, and binutils tools
- Spike reference ISS

The image is sufficient for the current Rust quality gate, project-authored
assembly/C/C++ guest programs, and Spike-based differential work. SystemC is not
part of the baseline image because the current crate has no native SystemC build
dependency. Add it through a reviewed image change when the SystemC adapter
becomes an active integration target.
...
workflow builds and verifies the image before publishing it to:
...
versions carrying a `sha-*` tag and deletes older `sha-*` versions. The rolling
`main` tag points at the newest of those versions. A deletion or package API
failure fails the workflow instead of silently bypassing retention.
...
Pull requests build and verify the image but never authenticate to GHCR or
publish a package. A manual workflow run publishes only when it runs against
the `main` branch. Publishing uses the workflow-scoped `GITHUB_TOKEN`; no
long-lived personal access token is required.
