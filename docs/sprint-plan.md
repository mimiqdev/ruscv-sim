...
| 10 | Week 19-20 | **内存子系统** | MMU、TLB、Sv39 页表遍历 | 160h |
| 11 | Week 21-22 | **TLM2.0 + 外设** | TLM 抽象层、CLINT、PLIC | 160h |
| 12 | Week 23-24 | **调试支持** | GDB RSP、断点、CLI | 140h |
| 13 | Week 25-26 | **集成测试** | 完整功能测试、回归测试 | 160h |
| 14 | Week 27-28 | **优化 & 发布** | 性能优化、风险缓冲、**v1.0 发布** | 140h |
| - | v1.0 后 | **[v1.0 后规划](#3105-v10-版本稳定后的规划)** | AIA、Sv48、多核等 | - |
...
### 3.10.5 v1.0 版本稳定后的规划

> **说明**: 本节列出的功能已明确推迟到 v1.0 稳定版本发布之后。当前阶段的核心目标是达成 v1.0 稳定版本（包含 RV64IMAFDC 指令集、基础内存子系统 Sv39、TLM2.0 外设框架和 GDB 调试支持）。以下高级特性将在 v1.0 稳定后根据实际需求重新评估优先级并规划实现。

---

#### RISC-V AIA (Advanced Interrupt Architecture) 支持

**状态**: ⏸️ 推迟至 v1.0 后  
**优先级**: v1.0 后重新评估  
**目标版本**: v1.1+  

**背景**: RISC-V AIA 是 RISC-V 特权架构规范中的高级中断架构扩展，提供了更灵活的中断投递机制和更丰富的中断控制器功能，包括 IMSIC (Incoming MSI Controller)、APLIC (Advanced Platform Level Interrupt Controller) 等组件。

**与现有 PLIC 的关系**:
- PLIC (Platform-Level Interrupt Controller) 是 v1.0 的基础中断控制器，支持传统中断方式
- AIA 是 PLIC 的现代化替代方案，提供更高效的 MSI (Message Signaled Interrupts) 支持
- v1.0 先保证 PLIC 完整功能，AIA 作为后续升级路径

**技术要点**:

| 组件 | 功能 | 复杂度 | 依赖 |
|------|------|--------|------|
| IMSIC | Per-hart 中断文件，支持 MSI | 高 | 多核支持框架 |
| APLIC | 平台级中断控制器，替代 PLIC | 中 | TLM2.0 外设框架 |
| HVICTL | 虚拟化中断控制 | 高 | 虚拟化扩展支持 |
| 中断投递 | 直接中断投递 (Direct Delivery) | 中 | 陷阱处理框架 |

**估计工时**: 80-120h (~2-3 Sprint)  
**参考规范**: RISC-V Advanced Interrupt Architecture Spec v1.0

---

#### 其他 v1.0 后的高级特性

以下特性同样推迟到 v1.0 稳定版本之后，将在 v1.0 后根据社区需求和实际应用场景重新评估优先级：

| 特性 | 状态 | 优先级 | 估计工时 | 目标版本 |
|------|------|--------|----------|----------|
| **Sv48/Sv57 虚拟内存** | 部分准备中 | 高 | 48h | v1.1 | 见 [8.4 节](#84-sv48sv57-虚拟内存支持) |
| **多核 (SMP) 支持** | 已知限制 | 高 | 160h | v1.2+ | 全局 reservation 需重构 |
| **64位原子操作 (AMO.D)** | 未实现 | 中 | 24h | v1.1 | 依赖 64-bit 内存接口 |
| **RV32I 纯32位仿真** | 待分析 | 低 | 88h | v1.2+ | 见 [8.5 节](#85-纯-32-位仿真支持-rv32i) |
| **虚拟化扩展 (H-Extension)** | 未实现 | 低 | 200h+ | v1.2+ | 需要 AIA 支持 |
| **性能优化 (SIMD/并行)** | 未实现 | 低 | 80h | v1.1+ | 并行指令执行 |
| **SystemC TLM2.0 桥接** | 未实现 | 低 | 60h | v1.1+ | 与 SystemC 生态集成 |
...
### 3.11 Sprint 11: TLM2.0 + 外设 (v1.0 最终阶段)

**目标**: 实现 Rust TLM2.0 抽象层、外设模型 (CLINT、PLIC)  
**里程碑**: v1.0 功能开发的最后一个 Sprint，完成后进入调试和测试阶段  

> **注意**: 高级特性如 RISC-V AIA 已推迟至 v1.0 稳定后，详见 [3.10.5 节](#3105-v10-版本稳定后的规划)
...
### Sprint 11.5: 边界测试和原子性改进

**完成时间**: 2026-02-02

**目标**: 补充 Sprint 11 的边界测试覆盖和原子性改进

#### 实际产出

**CLINT 原子性改进**:
- `src/peripherals/clint.rs`:
  - mtime: u64 → AtomicU64
  - 添加多线程安全注释
  - 更新读写方法使用 load/store 操作
...
#### 文件变更
- `src/peripherals/clint.rs`: +50 行
- `tests/clint_boundary_test.rs`: 新增 (30+ tests)
- `tests/plic_boundary_test.rs`: 新增 (30+ tests)
- `tests/uart_boundary_test.rs`: 新增 (30+ tests)
- `tests/tlm_exception_test.rs`: 新增 (20+ tests)
...
### 3.13 Sprint 13: 集成测试 (v1.0)

**目标**: 完整功能测试、回归测试、冒烟测试  
**阶段**: v1.0 测试验证阶段，确保 RV64IMAFDC 完整通过
...
### 3.14 Sprint 14: 优化 & 发布 (v1.0 GA)

**目标**: 性能优化、风险缓冲、**v1.0.0 正式发布**  
**里程碑**: 🎯 **v1.0.0 正式版本发布 (General Availability)**

**v1.0 发布标准**:
- [ ] RV64IMAFDC 指令集完整通过 RISC-V testsuite
- [ ] Sv39 虚拟内存稳定运行
- [ ] CLINT/PLIC 中断控制器功能完整
- [ ] GDB RSP 调试接口可用
- [ ] 代码覆盖率 > 80%
- [ ] 文档完整 (用户指南、API 参考)
...
| **M9: v1.0 发布** | Sprint 14 | v1.0.0 发布 | 🟢 低 |
| **M10: v1.1 规划** | v1.0 后 | AIA、Sv48、性能优化 | 🟡 中 | 见 [3.10.5 节](#3105-v10-版本稳定后的规划) |

### 4.1 v1.0 版本边界说明

**v1.0 核心范围**:
- ✅ RV64IMAFDC 基础指令集 (927+ 条指令)
- ✅ CSR 框架 + 特权模式 (M/S/U)
- ✅ Sv39 虚拟内存 + TLB
- ✅ CLINT + PLIC 中断控制器
- ✅ TLM2.0 抽象层
...
详细规划见: [3.10.5 v1.0 版本稳定后的规划](#3105-v10-版本稳定后的规划)

---

### 4.2 技术重构 Sprint (Tech Debt Sprints)
...
| **多核支持复杂** | 中 | 高 | 🔴 高 | ⏸️ 推迟至 v1.0 后 |
| **Rust TLM 性能** | 低 | 中 | 🟡 中 | 原型验证 |
| **Sv48 实现复杂度** | 低 | 低 | 🟢 低 | ⏸️ 推迟至 v1.0 后，分析完成 |
| **RV32I 架构重构** | 高 | 中 | 🟡 中 | ⏸️ 推迟至 v1.0 后，方案待确定 |
| **RISC-V AIA 规范变化** | 中 | 中 | 🟡 中 | ⏸️ 推迟至 v1.0 后，规范跟踪中 |

**推迟特性风险监控**:
- 这些特性已明确标记为 v1.0 后实现，不影响 v1.0 发布进度
- 需在 v1.0 开发期间保持接口设计的可扩展性，便于后续集成
- 定期跟踪 RISC-V 规范更新 (特别是 AIA 和虚拟化扩展)
...
| v3.7 | 2026-02-02 | - | **v1.0 版本边界明确化** |
| | | | - 新增 3.10.5 节: v1.0 版本稳定后的规划 |
| | | | - 明确推迟 RISC-V AIA 至 v1.0 后 (v1.1+) |
| | | | - 添加 v1.0 后高级特性清单 (Sv48、多核、RV32I 等) |
| | | | - 更新 Sprint 总览表，添加 v1.0 后规划行 |
| | | | - 更新里程碑计划 (M10: v1.1 规划) |
| | | | - 更新风险评估，标记推迟特性风险 |
| | | | - 添加 4.1 节: v1.0 版本边界说明 |
| v3.8 | 2026-02-02 | - | **添加 Sprint 11.5 完成记录** |
| | | | - 90+ 个边界测试 |
| | | | - CLINT 原子性改进 |
