...
**Authority:** Descriptive; source code and verified tests remain authoritative

**Last verified:** 2026-09-03

**Scope:** The public ELF execution path, adjacent library APIs, and the integration status of existing ISS/VP components

This document describes what the repository implements today. It is deliberately separate from the [target architecture](./README.md): target diagrams define intended ownership, while this inventory records current wiring and boundary debt.
...
flowchart TB
    ...
    EXEC --> ADAPTER["MemoryAdapter<br/>address minus ELF base"]
    ADAPTER --> BUS
...
- ELF memory is stored relative to its lowest load address. `SystemBus` converts physical addresses to RAM offsets, while `RiscvCore::MemoryAdapter` contains a second base-subtraction mechanism used by the flat-memory library path.
...
| RV64 instruction semantics | RV64I dispatch plus M/A/F/D paths in the active executor | **Public path, coverage varies** | Presence in the dispatcher is not a claim of full extension compliance; external architectural verification has not yet established the supported profile. |
...
| Native bus | Concrete RAM/UART/HTIF routing in `executor.rs` | **Public path and library API** | `SystemBus` is public through `ruscv_sim::executor`, with fixed devices and limited access widths; no reusable address-map/platform contract exists. |
| UART16550 | Register model and output callback | **Public path, limited wiring** | Public bus exposes byte accesses at `0x1000_0000`; the richer TLM target behavior is tested separately. |
| HTIF / `tohost` | Fixed write endpoint, callback, selected-address polling, exit decoding, and RAM clearing | **Public path** | The fixed `0x4000_8000` endpoint does not store register state and reads as zero; callback-based exit and polling of a selected ELF/CLI `tohost` address are separate mechanisms embedded in executor orchestration. |
| Commit trace | Optional per-retired-instruction record | **Public path** | Register deltas are recorded; memory access is explicitly passed as `None`, and instruction refetch has separate address handling. |
...
| Cached instruction dispatcher | `InstructionDispatcher`, instruction-key lookup, and LRU cache | **Component** | Focused tests cover registration and cache behavior, but `RiscvCore` owns and calls `Executor` directly; this dispatcher is not in the active execution path. |
...
| MMU / Sv39 / TLB | Translation types, page-table walker, TLB, A/D behavior, physical-memory model | **Component** | Extensively focused-testable, but no MMU is owned or called by `RiscvCore`; ELF base subtraction is the active address adaptation. |
