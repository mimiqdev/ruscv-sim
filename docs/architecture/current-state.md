...
| Physical access | `MemoryInterface` remains the active typed storage API; `src/physical.rs` now provides the A7 T1 transport-neutral non-atomic vocabulary and a validated backend boundary, but no public execution caller uses it | T2 must supply native targets/adapters and T3 must connect Hart ordinary accesses. Keep ISA load interpretation, alignment, translation, and privilege checks in Hart semantics; preserve the disclosed legacy atomic route. T1 is component evidence only, not public migration or all-access convergence. |
