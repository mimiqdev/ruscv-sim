...
| Runner/orchestration | Per-instruction loop, cycle limit, HTIF polling, signature dump, result construction | **Public path** | Implemented inside `load_and_run`, not as a stable Runner abstraction. Image placement, result construction, the run-control decision and image installation are shared with the library wrapper: one owner assembles the exit, cycle count, final PC, artifact and primary failure with an explicit artifact-failure policy, and one owner holds the instruction budget, the retirement count, the timeout diagnostic and the decode-before-clear exit rule, and one owner installs a loaded image — RAM creation, program load, core construction and entry-point reset — over the memory backend each configuration supplies. Each loop still owns its stepping, its own signal sources in its own order, its logging and its diagnostic wording, so a failure message can differ between the configurations even where the shape does not. |
