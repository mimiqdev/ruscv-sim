...
`d1834cc6566b342824bca30772cc829953a5c5ef`). The approved successor contract is
Milestone A6 (Machine-Mode Synchronous Trap Entry and Return, see
...
is Draft/non-normative and is not the active plan. Its revised scope migrates
fetch and ordinary loads/stores only; legacy AMO/LRSC behavior must be preserved
as explicit debt. Atomic capability rejection was rejected, not approved as a
migration shortcut. The complete revised contract still awaits approval.
...
| Physical access | `MemoryInterface` combines typed storage operations with RISC-V sign/zero-extension helpers | Implement accepted ADR-0002's transaction/fault contract; keep ISA load interpretation, alignment, translation, and privilege checks in Hart semantics. The revised A7 candidate proposes non-atomic public Hart access migration, with a disclosed legacy atomic compatibility route, not all-access convergence or activation. |
