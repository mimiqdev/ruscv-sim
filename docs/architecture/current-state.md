...
| `RiscVSimulator` wrapper | Flat-memory load/step/run API | **Library path** | It duplicates parts of `load_and_run` and does not use `SystemBus`, so it has no UART/HTIF device mapping. Its flat addresses remain storage offsets, and it converts image-declared RAM `tohost` metadata by subtracting the image base rather than transferring guest virtual-to-physical translation or device ability. It must not become a second architecture engine. |
