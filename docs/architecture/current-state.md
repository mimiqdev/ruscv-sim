...
| Runner/orchestration | Per-instruction loop, cycle limit, HTIF polling, signature dump, result construction | **Public path** | Implemented inside `load_and_run`, not as a stable Runner abstraction. Image placement is shared with the library wrapper; exit retention and artifact composition are still constructed separately in each entry point. |
