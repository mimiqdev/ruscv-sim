...
| Runner/orchestration | Per-instruction loop, cycle limit, HTIF polling, signature dump, result construction | **Public path** | Implemented inside `load_and_run`, not as a stable Runner abstraction. Image placement and result construction are shared with the library wrapper: one owner assembles the exit, cycle count, final PC, artifact and primary failure, and the artifact-failure policy is an explicit input so the CLI keeps its documented suppression while the flat library reports. Callers still supply their own diagnostic wording and final-PC choice, so a failure message can differ between the configurations even where the shape does not. |
