...
**Characterization fixture:** commits
`485b6891639b4c60603e74e1222b8bd95f75f446` and
`d82186c4b21d073a539e791b89c97a19b7e2aa58` (fixture HEAD; the latter records
the architectural SC encoding) add only
...
| A7-T0-04 | LR at `0xc0`; ordinary `SD` at the same address; SC at `0xc0`. LR at `0xd0`; `FSD` at the same address; SC. | Each three-instruction sequence retires. The final SC succeeds (`rd=0`) and overwrites the preceding scalar/FP store. | Preserve that ordinary integer and FP writes do not invalidate the current legacy reservation. This is a labelled regression, not the architectural reservation rule. |
| A7-T0-05 | LR at `0xe0`; `RiscVSimulator::write_mem(0xe0, 8 bytes)`; SC at `0xe0`. | The host byte-loop write is visible, then SC still retires successfully and writes its value. | Preserve host-write visibility and the absence of a reservation notification on the legacy path. Host writes remain outside the migrated ordinary-guest transaction guarantee. |
...
`d82186c4b21d073a539e791b89c97a19b7e2aa58`.
...
runtime proof for this T0 fixture is also:

```bash
git diff --name-status \
  9ee9c5f24c072779f06ce141b3340f953b614442..d82186c4b21d073a539e791b89c97a19b7e2aa58
