...
| Signature failure policy | read errors still suppressed by `.ok().flatten()` ([G-06](public-behavior-gaps.md#g-06--signature-read-errors-are-suppressed-in-run-results)) | explicit diagnostic through the existing error surface | A2 T3 tests |
