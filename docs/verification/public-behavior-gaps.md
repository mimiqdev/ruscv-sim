...
- **Implementation evidence:** All `load_and_run` result paths still call `dump_signature(...).ok().flatten()`, so a CLI read error becomes `None` while `signature_addr` may remain `Some`. `RiscVSimulator::signature_artifact` no longer suppresses: an unmappable declared region or a failed flat read leaves `signature_data` as `None` **and** appends an explicit diagnostic to `ExecutionResult.error`, without disturbing exit, cycles, final PC or a primary failure. A2's non-goals exclude redesigning the CLI signature-failure policy, so that path is unchanged and still cannot be advertised as successful capture.
