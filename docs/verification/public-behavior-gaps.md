...
### G-14 — Base-I FENCE (`MiscMem`, `funct3 = 0b000`) was unconditionally rejected

- **Disposition:** **Reproduced defect** and **Repaired** under A5 (in-scope base-I FENCE behavior; the milestone contract requires FENCE be made correct for synchronous, single-Hart ordered memory without inventing a concurrent platform model).
- **Surface:** [`InstructionDecoder::decode`](../../src/decode/mod.rs)'s `Opcode::MiscMem` arm and [`Executor::execute`](../../src/execute/mod.rs), on the shared decode+execute path used by the public ELF `run`.
- **Implementation evidence:** Before this fix, the decoder's `MiscMem` arm unconditionally returned `DecodeError::UnimplementedInstruction` for every encoding, including FENCE, and `Executor::execute` had no `MiscMem` arm at all. The fix reads the raw `funct3` bits directly (`(instruction >> 12) & 0x7`); when `funct3 == 0b000` (base-I FENCE) it decodes an I-type-shaped instruction with the raw, non-sign-extended `fm|pred|succ` field in `imm` and `rs1`/`rd` recorded only for trace/disassembly completeness, then dispatches to the new [`exec_fence`](../../src/isa/rv64i/fence.rs). `exec_fence` writes no register (including a nonzero `rd`, which the base spec requires implementations to ignore), performs no memory access, and never sets `branch_taken`, so the existing `RiscvCore::step` PC fall-through advances the PC by exactly 4. This is correct, not an approximation: `ruscv-sim` executes one Hart against a synchronous, in-order `MemoryInterface` with no cache, store buffer, or speculative reordering, so every prior instruction's memory effect is already globally visible before the next instruction issues, and FENCE (every `pred`/`succ`/`fm` shape, including `FENCE.TSO` and the reserved-`fm`/HINT cases the spec requires be treated identically to an ordinary fence) has nothing left to order. No ordering flags, barriers, or TLM hooks were added. `funct3 != 0b000` (`FENCE.I`/Zifencei at `0b001`, Zicbo CBO ops at `0b010`, and all other MISC-MEM `funct3` values) is untouched and stays rejected with `DecodeError::UnimplementedInstruction`.
- **Reproduction:** Before the fix, decoding any FENCE encoding (e.g. `0x0ff0000f`, the pinned ACT4 `fence` mnemonic) returned `UnimplementedInstruction`, and running the pinned `I-fence-00.S` self-check ELF through `ruscv-sim run` aborted with `Execution error at PC 0x0000000080000260: Unimplemented instruction` after 150 retired cycles (exit code 1). Unit reproduction: `src/isa/rv64i/fence.rs::tests::{all_i_fence_00_encodings_decode_and_execute_as_noop, all_i_fence_00_encodings_advance_pc_by_4_through_step}` observed `UnimplementedInstruction` panics for all 13 FENCE encodings the pinned `I-fence-00.S` source emits, before the fix. Public-path reproduction: `public_behavior::public_fence_between_ordinary_instructions_executes_as_a_noop` built an ELF with a FENCE between two ordinary instructions; before the fix it returned `exit_code=1` (aborted), after the fix `exit_code=42, cycles=4`.
- **Existing tests:** `src/isa/rv64i/fence.rs::tests::{all_i_fence_00_encodings_decode_and_execute_as_noop, all_i_fence_00_encodings_advance_pc_by_4_through_step, fence_i_is_not_base_i_and_stays_rejected}` are **strong** for every FENCE shape the pinned `I-fence-00.S` source emits (plain fence, `fence.tso`, reserved-but-must-still-execute `rs1`/`rd`/`fm` combinations, and `pred=0`/`succ=0` HINTs) on both the decode+execute path and the full `RiscvCore::step` fetch-decode-execute pipeline, plus the Zifencei boundary; `public_behavior::public_fence_between_ordinary_instructions_executes_as_a_noop` is **strong** for the full public ELF `run` path. No existing test was weakened or deleted.
- **Impact:** Every base-I FENCE encoding aborted guest execution with `Unimplemented instruction` the moment it was reached, including a bare `fence` with no explicit operands, `fence.tso`, and hint encodings. This is the proved defect and its direct regressions.
- **ACT4 selection replay (bounded, informative):** The retained A5 feasibility artifact `10274071547` (SHA-256 `08682429f62b8deaf656e1013dd1e11ad54795609353b48087f3810977cdbbd1`, `21676628` bytes) holds the generated `I-fence-00` self-check ELF plus the other 50 pinned RV64I selection ELFs and `evidence/generated-manifest.json`. Two release binaries were built from the same source tree differing only by this fix (`origin/main` at `978cce0a44a9a25a7ec6d7716b651a3da4f7c1de` for "before"; this PR's branch for "after") and used to replay all 51 unchanged ELFs (hashes verified against the manifest) through `target/release/ruscv-sim run <elf> --max-cycles 1000000`:

  | Case | ELF SHA-256 (prefix) | Before (`978cce0`) | After (this fix) |
  | --- | --- | --- | --- |
  | `I-fence-00` | `a0e8d5fe` | exit 1, FAILED, PC `0x80000260`, 150 cycles, `Unimplemented instruction` | exit 0, SUCCESS, 183 cycles, PC `0x80005038` |
