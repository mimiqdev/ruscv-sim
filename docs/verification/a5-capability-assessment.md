# A5 Capability Assessment — Navigation

**Status:** Current evidence navigation; A5 formally closed out.

**Authority:** Informational, not an active or successor milestone contract.

**Reconciled:** 2026-09-15

A5 has complete **selected external compatibility** evidence: 51 required
sources/variants generated, executed through the release public CLI and passed
at `f2edf77f17a76bea3d7062f40f5f4f38ad4eb580`. The maintainer confirmed the
MXLEN adapter and naturally aligned EEI decisions; no semantic input changed
after the successful 2026-09-13 run. This is not certification.

- [Standalone closeout](../archive/milestones/a5-closeout-record.md) — final
  dispositions, approval boundary and limitations.
- [Complete archived assessment](../archive/milestones/a5-capability-assessment.md)
  — all five criteria, source accounting, negative controls, exact reviews,
  final CI, reproducibility, artifact retrieval and residual limits.
- [Preserved approved contract](../archive/milestones/a5-act4-rv64i-external-compatibility.md)
  — historical contract body, not a second current plan.
- [Current forwarding contract](../dev-plan.md) — completed A5 remains current;
  no successor has been approved.
- [Frozen decision and original proposal](a5-selection-proposal.md).
- [All 1,970 source dispositions](a5-source-inventory.json).
- [Final 51-case evidence and controls](a5-final-results.json).
- [Original 44/51 failed-run record](a5-accounting-results.json) and
  [feasibility history](a5-feasibility.md) — retained, not current blockers.

PR #37 formally closed out A5 (merged `d1834cc6566b342824bca30772cc829953a5c5ef`)
with recorded independent closeout review and exact-head CI. The optional
single successor recommendation is not an approved A6; no successor contract
is approved.
