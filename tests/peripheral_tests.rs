...
fn test_clint_tlm_interface() {
    ...
    // Test read via TLM
    let mut read_trans =
        TlmGenericPayload::new(TlmCommand::Read, 0x0200_0000 + clint::reg_offset::MTIME, 4);
    delay = ScTime::zero();
...
fn test_uart_interrupt_handling() {
    ...
    assert_eq!(
        iir & uart16550::iir_bits::ID_MASK,
        uart16550::iir_bits::RECEIVE_DATA
    );
}
...
fn test_uart_tlm_interface() {
    ...
    let mut write_trans = TlmGenericPayload::with_data(TlmCommand::Write, 0x1000_0000, write_data);
    delay = ScTime::zero();
