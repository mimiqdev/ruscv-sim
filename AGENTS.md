...
- Read `docs/dev-plan.md` and the relevant repository ADRs before planning or implementing project work.
- `docs/dev-plan.md` contains exactly one current milestone contract: its objective, scope boundaries, non-goals, architectural constraints, deliverables, and acceptance criteria.
- Accepted cross-cutting architecture decisions live in repository ADRs and cannot be overridden by informal task descriptions or status changes.
- Source code and verified tests are authoritative for implemented behavior and integration claims; current documents must cite them rather than infer support from component presence.
- If a document conflicts with `docs/dev-plan.md`, an accepted ADR, or verified implementation evidence, stop and surface the concrete conflict instead of silently changing scope.
- When `README.md`, `CHANGELOG.md`, design documents, or archived plans conflict with the current milestone contract, an accepted ADR, or verified implementation evidence, follow the authoritative source and note the stale documentation if it affects the task.
- Treat `docs/archive/` as historical context only. Archived checkboxes, old version targets, and superseded frameworks are not current work.
- Keep durable repository documents self-contained; record change provenance in Git history rather than relying on private execution systems.
...
1. Verify every acceptance criterion in `docs/dev-plan.md` with recorded repository evidence.
...
5. Replace `docs/dev-plan.md` with the next approved milestone contract.
...
- Current milestone scope and architecture sequence: read from `docs/dev-plan.md`; use source code and verified tests for implementation status
